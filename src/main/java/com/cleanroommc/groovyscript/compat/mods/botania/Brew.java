...
public class Brew extends VirtualizedRegistry<vazkii.botania.api.brew.Brew> {
    ...
    @RecipeBuilderDescription(example = @Example(value = ".key('groovy_example_brew').name('Groovy Brew').color(0x00FFFF).cost(100).effect(new PotionEffect(potion('strength'), 1800, 3), new PotionEffect(potion('speed'), 1800, 2), new PotionEffect(potion('weakness'), 3600, 1)).incense(true).bloodPendant(true)", imports = "net.minecraft.potion.PotionEffect"))
