...
public class Compressor extends VirtualizedRegistry<MachineRecipe<IRecipeInput, Collection<ItemStack>>> {
    ...
    private void add(MachineRecipe<IRecipeInput, Collection<ItemStack>> recipe, boolean scripted) {
        Recipes.compressor.addRecipe(recipe.getInput(), recipe.getOutput(), recipe.getMetaData(), false);
    ...
    protected List<MachineRecipe<IRecipeInput, Collection<ItemStack>>> asList() {
        ...
        for (MachineRecipe<IRecipeInput, Collection<ItemStack>> rec : Recipes.compressor.getRecipes()) {
            list.add(rec);
        }
        return list;
    }
