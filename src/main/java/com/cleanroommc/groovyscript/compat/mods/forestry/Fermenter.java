...
public class Fermenter extends ForestryRegistry<IFermenterRecipe> {
    ...
    public IFermenterRecipe add(FluidStack output, FluidStack input, IIngredient catalyst, int value, float modifier) {
        ...
        if (catalyst instanceof OreDictIngredient) recipe = new FermenterRecipe(((OreDictIngredient) catalyst).getOreDict(), value, modifier, input.getFluid(), output);
    ...
    public boolean removeByCatalyst(IIngredient input) {
        if (FermenterRecipeManagerAccessor.getRecipes().removeIf(recipe -> {
            boolean found = input instanceof OreDictIngredient ? recipe.getResourceOreName().equals(((OreDictIngredient) input).getOreDict()) : recipe.getResource().isItemEqual(input.getMatchingStacks()[0]);
            if (found) addBackup(recipe);
            return found;
        })) return true;
    ...
    public class RecipeBuilder extends AbstractRecipeBuilder<IFermenterRecipe> {
        ...
        public @Nullable IFermenterRecipe register() {
            ...
            if (catalyst instanceof OreDictIngredient) recipe = new FermenterRecipe(((OreDictIngredient) catalyst).getOreDict(), value, modifier, fluidInput.get(0).getFluid(), fluidOutput.get(0));
