...
public class Macerator extends VirtualizedRegistry<MachineRecipe<IRecipeInput, Collection<ItemStack>>> {
    ...
    public void removeByOutput(ItemStack output) {
        ...
        for (Iterator<? extends MachineRecipe<IRecipeInput, Collection<ItemStack>>> iterator = Recipes.macerator.getRecipes().iterator(); iterator.hasNext(); ) {
            MachineRecipe<IRecipeInput, Collection<ItemStack>> recipe = iterator.next();
            if (ItemStack.areItemStacksEqual((ItemStack) recipe.getOutput().toArray()[0], output)) {
                iterator.remove();
                addBackup(recipe);
            }
        }
    ...
    private void add(MachineRecipe<IRecipeInput, Collection<ItemStack>> recipe, boolean scripted) {
        Recipes.macerator.addRecipe(recipe.getInput(), recipe.getOutput(), recipe.getMetaData(), false);
    ...
    protected List<MachineRecipe<IRecipeInput, Collection<ItemStack>>> asList() {
        ...
        for (MachineRecipe<IRecipeInput, Collection<ItemStack>> rec : Recipes.macerator.getRecipes()) {
            list.add(rec);
        }
        return list;
    }
