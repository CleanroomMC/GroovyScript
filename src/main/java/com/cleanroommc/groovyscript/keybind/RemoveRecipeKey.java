...
import com.cleanroommc.groovyscript.compat.mods.jei.removal.RemoveJEIRecipe;
...
public class RemoveRecipeKey extends GroovyScriptKeybinds.Key {
    ...
    public void handleKeybind() {
        ...
        if (Keyboard.getEventKeyState()) {
            var layout = JeiRemovalHelper.getRecipeLayoutUnderMouse();
            if (layout == null) return;
            var uid = JeiRemovalHelper.getFocusedRecipeUid();

            RemoveJEIRecipe.INSTANCE.checkRemoval(layout, uid, GuiScreen.isShiftKeyDown());
        }
    }
